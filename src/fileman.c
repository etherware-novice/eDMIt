...
uint32_t freadu32( FILE *source, bool bend )
{
	...
	fread(&retr, sizeof(unsigned char), 4, source);
...
bool fwriteu32( FILE *source, uint32_t num, bool bend )
{
	...
	return fwrite(&num, sizeof(unsigned char), 4, source) == 4;
}
...
unsigned char *funcompz( FILE *source, unsigned bytes, unsigned long *arrsize )
{
	...
	unsigned char *compressed = emalloc(sizeof(unsigned char) * bytes, "allocating space for uncompress");
	if( compressed == NULL ) return NULL;
	fread( compressed, bytes, sizeof(unsigned char), source );

	unsigned long dump = 32767;
	if( arrsize )
		dump = *arrsize;

	unsigned char *text = emalloc(sizeof(unsigned char) * dump, "allocating space to write out uncompressed text");
	uncompress( text, &dump, compressed, bytes );
...
unsigned long fwritecomp( FILE *source, const unsigned char *from, unsigned long fromLen )
{
	...
	unsigned long bufSize = compressBound( fromLen );
	unsigned char *buf = emalloc(sizeof(char) * bufSize, "allocating space for compress");
	if( !buf ) return 0;

	compress( buf, &bufSize, from, fromLen );
	fwrite( buf, sizeof(char), bufSize, source );
...
static void crcTableCheck(void)
{
	if(crcTable[0]) return;
...
static unsigned long updateCrc( unsigned long crc, const unsigned char *buf, int len )
{
	...
	crcTableCheck();
	for( n = 0; n < len; n++ ) c = crcTable[ (c ^ buf[n]) & 0xff ] ^ (c >> 8);

	return c;
}

unsigned long crc032( const unsigned char *buf, int len )
{
